...
const remove_long_hex_blobs = (sql: string) =>
	sql.replace(/\b0x[0-9a-f]+/g, (match) => '[' + (match.length / 2) + ' bytes]')

const log_query = (log_me: string, sql: string) =>
	console.log(
		`========================= ${log_me} =========================\n${
			un_indent(remove_long_hex_blobs(sql))
		}`,
	)
...
const is_mysql_error = (error: unknown): error is MysqlError =>
	typeof error === `object` &&
	!!error &&
	`sql` in error &&
...
export default () => {
	...
	return <T extends Connection>(mysql: T): T =>
		Object.create(mysql, {
			query: {
				async value(input_sql: string, values?: unknown[]) {
					const log_me = `query ${
						wrap_with_style(terminal_styles.yellow, next_query_number++)
					}`
					...
					try {
						...
						if (is_mysql_error(error)) {
							...
							console.error(
								wrap_with_style(terminal_styles.bright_red, un_indent(error.sql)),
							)
						}
